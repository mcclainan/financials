...
class RemoteFormTagLib {
    ...
    def remotePaginate = {attrs ->
        ...
        if(bootstrapEnabled){
            writer << '<ul class="pagination pagination-lg">'
        }
        ...
        if (steps && laststep > firststep) {
            ...
            if (beginstep > firststep) {
                ...
                writer << wrapInListItem(bootstrapEnabled,'<a href="#">..</a>')
            }
            ...
            (beginstep..endstep).each {i ->
                if (currentstep == i) {
                    ...
                    writer << wrapInListItem(bootstrapEnabled,"<a href=\"#\">${i}</a>",currentStepClass)
                } else {
                    linkParams.offset = (i - 1) * max
                    writer << wrapInListItem(bootstrapEnabled,remoteLink(linkTagAttrs.clone()) {i.toString()}).replace('class="step"','class=""')
                }
            }

            // display laststep link when endstep is not laststep
            if (endstep < laststep) {
                writer << wrapInListItem(bootstrapEnabled,'<span class="">..</span>')
    ...
    private def wrapInListItem(Boolean bootstrapEnabled, def val,String clazz){
        bootstrapEnabled ? "<li class=\"${clazz}\">$val</li>" : val
    }
}
